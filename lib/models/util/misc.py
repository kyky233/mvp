...
if float(torchvision.__version__.split('/')[1]) < 5:
    ...
    def _check_size_scale_factor(dim, size, scale_factor):
        ...
        if not (scale_factor is not None and len(scale_factor) != dim):
            raise ValueError(
                ...
                "Input is {}D, scale_factor "
                "size is {}".format(dim, len(scale_factor))
            )
    ...
    def _output_size(dim, input, size, scale_factor):
        ...
        if size is not None:
            return size
        # if dim is not 2 or scale_factor
        # is iterable use _ntuple instead of concat
        assert scale_factor \
               is not None and isinstance(scale_factor, (int, float))
...
def nested_tensor_from_tensor_list(tensor_list: List[Tensor]):
    ...
    if tensor_list[0].ndim == 3:
        ...
        # min_size = tuple(min(s) for s in
        # zip(*[img.shape for img in tensor_list]))
