...
class JointsDataset(Dataset):
    ...
    def __init__(self, cfg, image_set, is_train, transform=None):
        ...
        dataset_root = os.path.join('/mntnfs/med_data5/wangjiong/datasets', cfg.DATASET.ROOT)
    ...
    def __getitem__(self, idx):
        ...
        if isinstance(self.root_id, int):
            ...
            roots_3d = np.mean([joints_3d_u[:, j]
                                for j in self.root_id], axis=0)
    ...
    def compute_human_scale(self, pose, joints_vis):
        ...
        return np.clip(np.maximum(maxy - miny, maxx - minx)
                       ** 2, 1.0 / 4 * 96 ** 2, 4 * 96 ** 2)
    ...
    def generate_3d_target(self, joints_3d):
        ...
        grid1Dx = np.linspace(-space_size[0] / 2,
                              space_size[0] / 2,
                              cube_size[0]) + space_center[0]
        grid1Dy = np.linspace(-space_size[1] / 2,
                              space_size[1] / 2,
                              cube_size[1]) + space_center[1]
        grid1Dz = np.linspace(-space_size[2] / 2,
                              space_size[2] / 2,
                              cube_size[2]) + space_center[2]

        target = np.zeros((cube_size[0],
                           cube_size[1],
                           cube_size[2]), dtype=np.float32)
        cur_sigma = 200.0
        ...
        for n in range(num_people):
            ...
            if isinstance(joint_id, int):
                ...
                mu_x = (joints_3d[n][joint_id[0]][0]
                        + joints_3d[n][joint_id[1]][0]) / 2.0
                mu_y = (joints_3d[n][joint_id[0]][1]
                        + joints_3d[n][joint_id[1]][1]) / 2.0
                mu_z = (joints_3d[n][joint_id[0]][2]
                        + joints_3d[n][joint_id[1]][2]) / 2.0
            ...
            for n in range(nposes):
                ...
                for joint_id in range(num_joints):
                    ...
                    if 'campus' in self.dataset_name:
                        ...
                        max_value = joints[n][joint_id][2] \
                            if len(joints[n][joint_id]) == 3 else 1.0
                        # max_value = max_value**0.5
                    g = np.exp(
                        -((x - x0) ** 2 + (y - y0) ** 2)
                        / (2 * cur_sigma ** 2)) * max_value
